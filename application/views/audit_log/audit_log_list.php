...
<div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden">
    ...
    <!-- Mobile Cards -->
    <div class="block md:hidden p-4 sm:p-6 bg-slate-50/50 space-y-3 sm:space-y-4">
        <?php if (!empty($logs)): ?>
            <?php foreach ($logs as $l): ?>
                <div class="p-4 sm:p-5 bg-white border border-slate-200 rounded-2xl shadow-sm">
                    <!-- Header: admin + badge aksi -->
                    <div class="flex items-start justify-between gap-3 mb-3">
                        <div class="flex items-center space-x-3 min-w-0">
                            ...
                            <div class="min-w-0">
                                <h4 class="font-bold text-slate-800 tracking-tight leading-snug truncate"><?php echo htmlspecialchars($l['nama_admin'] ?: 'Tidak diketahui'); ?></h4>
                                <p class="text-[11px] text-slate-400 font-medium truncate">@<?php echo htmlspecialchars($l['username'] ?: '-'); ?></p>
                            </div>
                        </div>
                        <span class="text-[10px] font-bold px-2.5 py-1 rounded-full flex-shrink-0 <?php echo aksi_badge_class($l['aksi']); ?>">
                            <?php echo aksi_label($l['aksi']); ?>
                        </span>
                    </div>

                    <!-- Info grid: waktu + modul -->
                    <div class="grid grid-cols-2 gap-3 text-xs border-y border-slate-100 py-3 my-2">
                        <div>
                    ...
                    <!-- Deskripsi -->
                    <p class="text-xs text-slate-600 leading-relaxed"><?php echo htmlspecialchars($l['deskripsi']); ?></p>
                    <p class="text-[10px] text-slate-400 mt-2"><i class="fa-solid fa-globe mr-1"></i>IP: <?php echo htmlspecialchars($l['ip_address']); ?></p>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
